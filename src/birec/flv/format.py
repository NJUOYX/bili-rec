"""FLV binary format parser and dumper."""
...
class FlvParser:
    """Parse FLV binary data into models."""
    ...
    def parse_previous_tag_size(self) -> int:
        """Parse previous tag size (back pointer)."""
    ...
    def parse_tag(self, *, no_body: bool = False) -> FlvTag:
        ...
        if tag_header.tag_type == TagType.AUDIO:
            ...
            return AudioTag(
                offset=offset,
                filtered=tag_header.filtered,
                tag_type=tag_header.tag_type,
                data_size=tag_header.data_size,
                timestamp=tag_header.timestamp,
                stream_id=tag_header.stream_id,
                sound_format=audio_tag_header.sound_format,
                sound_rate=audio_tag_header.sound_rate,
                sound_size=audio_tag_header.sound_size,
                sound_type=audio_tag_header.sound_type,
                aac_packet_type=audio_tag_header.aac_packet_type,  # type: ignore[arg-type]
                body=body,
            )
            ...
            return VideoTag(
                ...
                timestamp=tag_header.timestamp,
                stream_id=tag_header.stream_id,
                frame_type=video_tag_header.frame_type,
                codec_id=video_tag_header.codec_id,
                avc_packet_type=video_tag_header.avc_packet_type,  # type: ignore[arg-type]
                composition_time=video_tag_header.composition_time or 0,
                body=body,
            )
...
class FlvDumper:
    """Dump FLV models to binary format."""
